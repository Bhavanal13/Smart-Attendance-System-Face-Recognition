...
mail = 'projectmatlab834@gmail.com'; 
password = 'Project.1234';

s1_email = 'hvsinbox@gmail.com';
s2_email = 'gvs1216@gmail.com';
...
while double(get(fdrFig,'currentCharacter')) ~= 27 && ~isDone
    ...
    if (tcount > 100)
        if send_mail == 0
            if s1 == 0
                sendmail(s1_email,'Mail from MATLAB College','Hello! This is to inform you that your ward -Person 1 is absent')
                disp('mail sent to Parent-1')
            end
    
            if s2 == 0
                sendmail(s2_email,'Mail from MATLAB College','Hello! This is to inform you that your ward -Person 2 is absent')
                disp('mail sent to Parent-2')
            end
	...
	for jj = 1:size(bboxes,1)%#ok
		if all(bboxes(jj,3:4) >= QC.minBBSize)
            ...
            tdt = datestr(now,'HH:MM:SS');
            ttime = datestr(now,'dd/mm/yyyy');

            %disp('ID number of PIC');            
            id=sprintf('%1.0f',bestGuess);
            %disp(id);
            ...
            if( id == '1')  
                if s1 == 0
                    ...
                    disp(datestr(now));
                     ...
                     fid = fopen('Attendance.txt', 'a'); % This portion of code writes the number plate
                     fprintf(fid,'Person 1 ID:%s - %s - %s\n',id,ttime,tdt);     % to the text file, if executed a notepad file with the    
                end
            end
